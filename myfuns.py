...
# The following functions take a time-series and modify their time-domain
# frequencies.
# In particular, they take an input and decrease the frequency.
# For example, they take a monthly time series and transform it to quarterly.
# Several aggregation methods are offered: average, max, min, first, last.
# A utility to decide what to do with NaNs is offered too.

# def monthlify(y, input_frequency, aggregation='average', discard_nans=False):
#     """
#     Parameters
#     ----------
#     y : float, numpy.ndarray
#          The time series of interest. It can be a vector or a matrix.
#          It must satisfy y.shape = (T,) or y.shape = (T,1), or an error is
#          thrown.
#     input_frequency : str {'hourly', 'daily', 'weekly'}
#                       Instructs the program on how to treat the input.
#                       Depending on the provided value, each successive entry
#                       of the vector y is assumed to occur one (hour/day/week)
#                       after. Missing values should be reported as NaNs.
#     aggregation : str, {'average', 'sum', 'max', 'min', 'first', 'last'}
#                   It defines how the data are supposed to be aggregated.
#                   Values are self-explanatory. Default value is 'average'.
#     discard_nans : boolean
#                    This defines what has to be done if NaNs are present in the
#                    series.
#                    If True, then NaNs are simply ignored by the aggregation
#                    method, unless all aggregated values are NaNs. If False,
#                    then NaNs propagate to the output.
#     """
#
#
# def quarterlify(y, input_frequency, aggregation='average', discard_nans=False):
#     """
#     Parameters
#     ----------
#     y : float, numpy.ndarray
#          The time series of interest. It must be defined on a weekly, daily or
#          higher frequency.
#          It must satisfy y.shape = (T,) or y.shape = (T,1), or an error is
#          thrown.
#     input_frequency : str {'hourly', 'daily', 'weekly', 'monthly'}
#                       Instructs the program on how to treat the input.
#                       Depending on the provided value, each successive entry
#                       of the vector y is assumed to occur one (hour/day/week/
#                       month) after. Missing values should be reported as NaNs.
#     aggregation : str, {'average', 'sum', 'max', 'min', 'first', 'last'}
#                   It defines how the data are supposed to be aggregated.
#                   Values are self-explanatory. Default value is 'average'.
#     discard_nans : boolean
...
#
#
# def yearlify(y, input_frequency, aggregation='average', discard_nans=False):
#     """
#     Parameters
#     ----------
#     y : float, numpy.ndarray
...
#          It must satisfy y.shape = (T,) or y.shape = (T,1), or an error is
#          thrown.
#     input_frequency : str {'hourly', 'daily', 'weekly', 'monthly', 'quarterly'}
#                       Instructs the program on how to treat the input.
#                       Depending on the provided value, each successive entry
#                       of the vector y is assumed to occur one (hour/day/week/
#                       month/quarter) after. Missing values should be reported
#                       as NaNs.
#     aggregation : str, {'average', 'sum', 'max', 'min', 'first', 'last'}
#                   It defines how the data are supposed to be aggregated.
#                   Values are self-explanatory. Default value is 'average'.
...
# def hpfilter(y, frequency):
#     """
#     This function implements the Hodrick-Prescott filter
#
#     Parameters
#     ----------
#     y : float, numpy.ndarray
#          The time-series of interest. It must satisfy y.shape = (T,) or
#          y.shape = (T,1), where T is the length of the time-series.
#     frequency : {'year', 'quarter', 'month'} or float
#                 Sets the free parameter 'lambda' (L) in the minimization
#                 problem. Preset options are available (str). If a number is
#                 specified, the preset options are overridden. The presets are
#                 taken from [HP] and [RU].
#
#     References
#     ----------
#     .. [HP] Hodrick, R.J. and Prescott, E.C. (1997), "Postwar U.S. Business
#             Cycles: An Empirical Investigation." Journal of Money, Credit and
#             Banking. 29(1), 1-16
#     .. [RU] Ravn, M. and Uhlig, H. (2002), "On adjusting the Hodrick-Prescott
#             filter for the frequency of observations." The Review of Economics
#             and Statistics. 84(2), 37.
#     """
#     if y.ndim > 2:
#         raise ValueError('Input data is not a vector-like array')
#     elif y.ndim == 2:
...
#         L = 14400
#     else:
#         raise ValueError('\'frequency\' is an invalid input.')
#
#     print('Incomplete Function [HP filter]')


# def perturb(y, A, x, order=1):
#     """
#
#     :param y:
#     :param A:
